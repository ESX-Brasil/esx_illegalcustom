			...
			if v.modType ~= nil then
				...
				if v.modType == 16 then
					...
					for j = 0, modCount, 1 do
						...
						if j == currentMods[k] then
							_label = 'nível ' .. j+1 .. ' - <span style="color:cornflowerblue;">'.. _U('installed') ..'</span>'
						else
							price = math.floor(vehiclePrice * v.price[j+1] / 100)
							_label = 'nível ' .. j+1 .. ' - <span style="color:green;">R$' .. price .. ' </span>'
						end
				...
				elseif v.modType == 17 then -- TURBO
					...
					if currentMods[k] then
						...
						_label = 'Turbo - <span style="color:green;">R$' .. math.floor(vehiclePrice * v.price[1] / 100) .. ' </span>'
					end
...
-- Ativar menu quando o jogador está dentro do marcador
Citizen.CreateThread(function()
	while true do
		Wait(0)
		local playerPed = GetPlayerPed(-1)
